...
void time_test()
{
	...
	volatile int x;
	int loop = 10000000;
	dlib::normarized_int norm_int(1, 1023);
	...
	std::cout << "f()          norm_int    :: " << ((double)(end - start) / CLOCKS_PER_SEC) << '\n';
	...
	for(int i = 0; i < loop; ++i)
	{
		norm_int_p2.fforward();
	}
	end = clock();
	std::cout << "fforward()   norm_int_p2 :: " << ((double)(end - start) / CLOCKS_PER_SEC) << '\n';
	...
	std::cout << "i&(length - 1)           :: " << ((double)(end - start) / CLOCKS_PER_SEC) << '\n';
}
